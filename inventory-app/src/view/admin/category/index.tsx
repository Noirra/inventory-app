...
export default function AdminCategory() {
  ...
  const handleCategorySuccess = async () => {
    setIsProcessing(true); // <-- loading saat simpan
    await fetchData();
    setShowModal(false);
        ...
        <div className={`bg-white p-6 rounded-2xl shadow border transition-all duration-200 ${isVisible ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}>
          ...
          <div className="flex justify-between mb-4">
            ...
            <button
              ...
              className={`bg-green-500 text-white px-4 py-2 rounded-lg flex items-center space-x-2 hover:bg-green-600 ${isProcessing ? "opacity-50 cursor-not-allowed" : ""}`}
